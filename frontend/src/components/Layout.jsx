...
import { ArrowUp, Car, CreditCard, Gift, Home, PiggyBank, ShoppingCart, Utensils, Zap, Activity, DollarSign, TrendingUp, ArrowDown, Clock, RefreshCcw, Info, ChevronLeftSquareIcon, ChevronsUp, ChevronsDown, PieChart } from 'lucide-react';
...
const Layout = ({ onLogout, user }) => {
  ...
  //to delete transaction from server
  ...
  ); // to filter transactions based on selected time frame

  // to calculate stats for dashboard
  const stats = useMemo(() => {
    const now = new Date();
          ...
          <div className={styles.grid.rightColumn}>
            <div className={styles.cards.base}>
              <div className={styles.transactions.cardHeader}>
                ...
                <button
                  onClick={fetchTransactions} disabled={loading}
              ...
              <div className={styles.transactions.dataStackingInfo}>
                ...
                <span>Transactions are stacke by date (newest first)</span>
              </div>

              <div className={styles.transactions.listContainer}>
                {displayedTransactions.map((transactions) => {
                  const { id, description, amount, date, category, type } = transactions;
                  return (
                      ...
                      <div className=" flex items-center gap-1 md:gap-4 lg:gap-3">
                        <div className={'p-2 rounded-lg ${styles.colors.transaction.bg(type)}'}>
                          {CATEGORY_ICONS[category] || (
                            <DollarSign className={styles.colors.transaction.icon} />
                          )}
                        </div>
                ...
                {transactions.length === 0 ? (
                  ...
                  <div className={styles.transactions.viewAllContainer}>
                    <button onClick={() => setShowAllTransactions(!showAllTransactions)} className={styles.transactions.viewAllButton}>
                      {showAllTransactions ? (
                        <>
                          <ChevronsUp className=" w-5 h-5" /> Show Less
                        </>
                      ) : (
                        <>
                          <ChevronsDown className=" w-5 h-5" /> View All Transactions ({transactions.length})
                        </>
                      )}
                    </button>
                  </div>
            ...
            <div className={styles.cards.base}>
              <h3 className={styles.cards.title}>
                <PieChart className={styles.categories.titleIcon} />                  Spending by Category
              </h3>
